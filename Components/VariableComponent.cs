...
namespace Org.Reddragonit.MustacheDotNet.Components
{
    ...
    var text=document.createTextNode(({0}==undefined ? '' : {0}.join('{1}')));
    ...
    for(var i=0;i<{0}.length;i++){{
        var pre = document.createElement('pre');
        var text=document.createTextNode(({0}[i]==undefined ? undefined : ({0}[i].{1}!=undefined ? {0}[i].{1} : ({0}[i].get != undefined ? {0}[i].get('{1}') : undefined)))+(i+1<{0}.length ? '{2}' : ''));
        pre.appendChild(text);
        ret+=pre.innerHTML;
    }}
}}else if ({0}.at!=undefined){{
    for(var i=0;i<{0}.length;i++){{
        var pre = document.createElement('pre');
        var text=document.createTextNode(({0}.at(i)==undefined ? undefined : ({0}.at(i).{1}!=undefined ? {0}.at(i).{1} : ({0}.at(i).get != undefined ? {0}.at(i).get('{1}') : undefined)))+(i+1<{0}.length ? '{2}' : ''));
        pre.appendChild(text);
        ret+=pre.innerHTML;
    }}
}}", new object[] { subVar, subVarField, repeater });
                    ret += string.Format(@"else{{
    var pre = document.createElement('pre');
    var text=document.createTextNode(({0}==undefined ? '' : ({0}==null ? '' : {0}.toString()+'{1}')));
    pre.appendChild(text);
    ret+=pre.innerHTML;
}}", new object[] { var, repeater });
                }
